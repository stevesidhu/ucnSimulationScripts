...
//Experimental constants used in simulation and analysis
    double emptyingTime = 55;  //cell emptying time
...
struct Study{
    ...
    std::vector< std::pair<double,double>> cellCuts;

};

typedef double (SimResult::*ResultMember);
typedef TH1F* (SimResult::*PlotMember);
...
void transport(std::string FileName, int modeParameter, SimResult &results, std::pair<double,double> cellcuts);

//Returns number of days to reach
double days(int mode, SimResult &result);

//SimulationResults
auto ccPair = std::make_pair(0.2,-0.2);
void analyzeSim(SimResult &results, std::string fileName, int mode, std::pair<double,double> cellcuts = ccPair);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //Tester study
    Study test_study2 = {"Tester",{35,95},{"newKinkHeight35.root","newKinkHeight95.root"},{},{{0.2,-0.2},{0.2,-0.2}}};
    Study test_study1 = {"Tester",{35},{"newKinkHeight35.root"},{},{{0.2,-0.2}}};
    ...
    Study newkinkHeight_study{"KinkHeight",{35,45,55,65,75,85,95},{"newKinkHeight35.root", "newKinkHeight45.root","newKinkHeight55.root","newKinkHeight65.root","newKinkHeight75.root","newKinkHeight85.root","newKinkHeight95.root"},{}};
    ...
    Study DoubleBKinkHeight_study{"2BKinkHeight",{35,45,55,65,75,85,95},{"2BkinkHeight35.root", "2BkinkHeight45.root","2BkinkHeight55.root","2BkinkHeight65.root","2BkinkHeight75.root","2BkinkHeight85.root","2BkinkHeight95.root"},{}};
    ...
    //minimal seperation vs kink height study: 5cm overfill and 50cm long HEX
    Study MinSepKinkHeight_study{"MinSepKinkHeightV1",{35,45,55,65,75,85,95},{"minSepKinkHt35.root","minSepKinkHt45.root","minSepKinkHt55.root","minSepKinkHt65.root","minSepKinkHt75.root","minSepKinkHt85.root","minSepKinkHt95.root"},{}};

    //Angled kink simulation, measured by total gain: 5cm overfill and 50cm long HEX
    Study AngledKinkHt_study{"AngledKinkHtStudy",{45,65,75,95,45,65,75,95},{"45cmGain45cmKinkHeight45.root","65cmGain65cmKinkHeight65.root","75cmGain75cmKinkHeight75.root","95cmGain95cmKinkHeight95.root","45cmGain27cmKinkHeight27.root","65cmGain27cmKinkHeight27.root","75cmGain27cmKinkHeight27.root","95cmGain27cmKinkHeight27.root"},{}};

///////////////////////////////////////////////////////////////////////////////////////////////////////////    
    
   //Tester function 
   std::vector<Study> allStudies = {test_study2};    
   ...
   //Specific studies
   // std::vector<Study> allStudies = {MinSepKinkHeight_study};
   // std::vector<Study> allStudies = {newkinkHeight_study};

 //   TFile outputFile("ResultsFile.root","RECREATE");
 //   TTree studyTree("studies","Tree containing study");

    //Run analysis functions on all simulations in each study and plot graphs
    for(Study study : allStudies){
        SimResult tempResult;
 //       studyTree.Branch("parametername", &study.parameter_name);
 //       studyTree.Branch("results", &tempResult);
        for(int i = 0; i < study.filenames.size(); ++i) {
            if(study.cellCuts.size() > 0){
                analyzeSim(tempResult, study.filenames[i], mode, study.cellCuts[i]);
            }
            else{
...
///////////////////////
        ...
        resultsFile <<"file, parameter,totalProducedReal, fillTime, fillTimeError, efficiency, efficiencyError, sourceStorageLifetime, sourceStorageLifetimeError, systemTau, uSystemTau,        totalUCNinSim, uTotalUCNinSim, numInCellReal, uNumInCellReal, UCNdensityInCells, transportEff, uTransportEff, avgTauCells, uavgTauCells, weightedDetEff, uWeightedDetEff, topCellRamseyS       urvivorsSim, bottomCellRamseySurvivorsSim, topCellFilledReal, topMeanEFill, uTopMeanEFill, bottomCellFilledReal, bottomMeanEFill, uBottomMeanEFill, topCellFinishedReal, topMeanEFinish,        uTopMeanEFinish, bottomCellFinishedReal, bottomMeanEFinish, uBottomMeanEFinish, topCellDetectedReal, topMeanEDetected, uTopMeanEDetected, bottomCellDetectedReal, bottomMeanEDetected, uB       ottomMeanEDetected, totalDetected, perFilledFromProduced, perDetectedFromProduced, perDetectedFromFilled, perTopCellDetectedFromProduced, perBottomCellDetectedFromProduced, optimalT_edm       , optimalEmptyingTime, daysToReach, errDaysToReach \n";

        for(int i = 0; i < study.filenames.size(); ++i) {
        resultsFile <<study.filenames[i] << ", "<< study.results[i].parameter <<", " << study.results[i].totalProducedReal <<", "<< study.results[i].fillTime << ", " <<study.results[i].fillTimeError << ", " <<study.results[i].efficiency << ", " << study.results[i].efficiencyError << ", " << study.results[i].sourceStorageLifetime << ", " << study.results[i].sourceStorageLifetimeError << ", " << study.results[i].systemTau << ", " << study.results[i].uSystemTau << ", " << study.results[i].totalUCN << ", " << study.results[i].uTotalUCN << ", " << study.results[i].numInCell << ", " << study.results[i].uNumInCell << ", " << study.results[i].numInCell/cellVolume << ", "<< study.results[i].transportEff << ", " << study.results[i].uTransportEff << ", " << study.results[i].avgTauCells << ", " << study.results[i].uavgTauCells << ", "<<study.results[i].WDetEff<< ", " << study.results[i].uWDetEff<< ", " <<study.results[i].topCellRamsey << ", " <<study.results[i].bottomCellRamsey << ", "  <<study.results[i].topCellTotalFilled << ", "  <<study.results[i].topMeanFill<< ", " << study.results[i].uTopMeanFill << ", " <<study.results[i].bottomCellTotalFilled << ", " <<study.results[i].bottomMeanFill << ", "<<study.results[i].uBottomMeanFill << ", " <<study.results[i].topCellTotalFinished << ", "<<study.results[i].topMeanFinish << ", "<<study.results[i].uTopMeanFinish << ", "<<study.results[i].bottomCellTotalFinished << ", "<<study.results[i].bottomMeanFinish << ", "<<study.results[i].uBottomMeanFinish << ", "<<study.results[i].topCellTotalDetected << ", "<<study.results[i].topMeanDetected << ", "<<study.results[i].uTopMeanDetected << ", " <<study.results[i].bottomCellTotalDetected << ", "<<study.results[i].bottomMeanDetected << ", "<<study.results[i].uBottomMeanDetected << ", "<< study.results[i].topCellTotalDetected + study.results[i].bottomCellTotalDetected <<", "<<study.results[i].numInCell/ study.results[i].totalProducedReal << ", "<< (study.results[i].topCellTotalDetected + study.results[i].bottomCellTotalDetected)/study.results[i].totalProducedReal <<", "<< (study.results[i].topCellTotalDetected + study.results[i].bottomCellTotalDetected)/study.results[i].numInCell<<", "<< study.results[i].topCellTotalDetected/study.results[i].totalProducedReal <<", "<< study.results[i].bottomCellTotalDetected/study.results[i].totalProducedReal<<", " << study.results[i].optimalTedm << ", "<< study.results[i].optimalEmptyingTime << ", "<< study.results[i].daysToReach << ", " << study.results[i].errDaysToReach << "\n";}
        resultsFile.close();      
    }
...
void transport(std::string fileName, int modeParameter,SimResult &results, std::pair<double,double> cellcuts)
{
    ...
    double cellUpperZBound = cellcuts.first;
    double cellLowerZBound = cellcuts.second;
...
//    filling->Fit(&fillTimeFit, "R");
    ...
    results.topMeanFill = topcellESpectrum->GetMean();
    results.bottomMeanFill = bottomcellESpectrum->GetMean();
    delete d;
...
std::vector<double> UCNremainingAfterCycle(TH1F *topspectrum, TH1F *bottomspectrum){
   ...
   // TFile topemptyingfile("minSepEmpt1VTop_DetEff.root");
   // TFile bottomemptyingfile("minSepEmpt1VBottom_DetEff.root");
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void analyzeSim(SimResult &result, std::string fileName, int mode, std::pair<double,double> cellcuts)
{
         std::cout << fileName <<" , " << mode << std::endl;
         std::string fileNameString = fileName.substr(0, fileName.size()-5);
	     std::vector<double> cellNumber;
	     transport(fileName, mode, result,cellcuts);
...
//        topFinish.GetYaxis()->SetRangeUser(0, 7E5);
//        topFinish.GetXaxis()->SetRangeUser(0, 250E-9);
//        TGaxis::SetMaxDigits(3);;
    ...
    TCanvas *can12 = new TCanvas("can11","TopDetected",3000,2000);
...
//        topFinishP->GetYaxis()->SetRangeUser(0, 7E5);
//        topFinishP->GetXaxis()-> SetRangeUser(0, 250E-9);
//        TGaxis::SetMaxDigits(3);;
    can12->Update();
...
//        bottomFinish.GetYaxis()->SetRangeUser(0, 7E5);
//        bottomFinish.GetXaxis()->SetRangeUser(0, 250E-9);
//        TGaxis::SetMaxDigits(3);;
    can21->Update();
...
//        bottomFinishP->GetYaxis()->SetRangeUser(0, 7E5);
//        bottomFinishP->GetXaxis()-> SetRangeUser(0, 250E-9);
//        TGaxis::SetMaxDigits(3);;
    can22->Update();
