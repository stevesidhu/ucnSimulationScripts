...
///////// Drawing energy spectrum in both cells combined.
	...
	if(mode == 0 || mode == 1) {
		...
		uTransportEff = abs(numInCell/numTransported) * sqrt(pow(uNumInCell/numInCell,2) + pow(uNumTransported/numTransported,2));  //confirmed by SS: 2019-02-28
		}
    else{
    transportEff =0;
    uTransportEff=0;
    ...
    efficiencyError = efficiency * sqrt(pow(uNumInCell/numInCell,2) + pow(uTotalUCN/totalUCN,2));  //confirmed by SS: 2019-02-28	
...
double days(int mode, SimResult &result)
{
    ...
    if (mode ==1)
        ...
        //cyclesPerDay = (stableField * 60)/((fillsPerCycle * (std::max(result.emptyingTime, sourcePumpingTime) +result.optimalTedm) + polarityTime)/60) -1 ;
    else
        cyclesPerDay = (stableField * 60)/((fillsPerCycle * (result.fillTime + result.emptyingTime + result.optimalTedm +sourcePumpingTime) + 2*polarityTime + degaussingTime/10)/60);
...
void analyzeSim(SimResult &result, std::string fileName, int mode, double cellCenter)
{
        ...
        //optimize t_edm to minimize the number of days
         TF1 optimizeddays_tEDM("optdays_tEDM",[mode, &result](double *x, double *p){
                 result.optimalTedm = x[0];
                 UCNremainingAfterCycle(result);
                 double d = days(mode, result);
                 return d;
            }, 20, 500, 0);
         optimizeddays_tEDM.SetNpx(1000);
         //plot the minimization and variance of t_edm
	     TCanvas *u = new TCanvas("u", "u", 800, 600);
         ...
         TF1 optimizeddays_empty("optdays_empty",[mode, &result](double *x, double *p){
                 result.emptyingTime = x[0];
                 UCNremainingAfterCycle(result);
                 double d = days(mode, result);
                 return d;
            }, 10, 200, 0);
         optimizeddays_empty.SetNpx(1000);
         //plot minimization and variance in emptying time
	     TCanvas *v = new TCanvas("v", "v", 800, 600);
         optimizeddays_empty.Draw();
         v->Print((fileNameString + "_optimized_day_empty.eps").c_str());
         result.emptyingTime = optimizeddays_empty.GetMinimumX(10,200);
         result.uEmptyingTime = (200.-10.)/1000.;
         std::cout << "\nOptimal emptyingTime: " <<  result.emptyingTime << " Optimal t_EDM time is: "<< result.optimalTedm <<"s\n";
         delete v;
         
         //Recalculate collection efficiency and UCNRemaining with optimized t_edm and emptyingTime
         UCNremainingAfterCycle(result);
...
double numError(int mode, SimResult tempResult)
{
    ...
    const int n = 6; //number of parameters    

    double error = 0;
    double errorPar[n] ={result.efficiencyError, result.fillTimeError, result.sourceStorageLifetimeError, result.dSurvivalprob,result.uEmptyingTime,result.uOptimalTedm}; 
    ...
    TF1 f("f",[mode,&result](double *x, double *p){
                ...
                result.survivalprob =p[3];
                result.emptyingTime = p[4];
                result.optimalTedm = p[5];
